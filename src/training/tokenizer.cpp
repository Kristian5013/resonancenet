...
void Tokenizer::init_byte_mappings() {
    // GPT-2 byte-to-unicode: printable ASCII + Latin-1 supplement get identity mapping,
    // remaining bytes get mapped to Unicode offset 256+.
    int n = 0;
    for (int b = 0; b < 256; ++b) {
...
Result<void> Tokenizer::load(const std::filesystem::path& vocab_path) {
    ...
    if (!vf.is_open()) {
        // Try treating vocab_path as the directory itself
        vf.open(vocab_path);
        if (!vf.is_open()) {
    ...
    if (std::getline(mf, line)) {
        // Check if it's actually a merge line (no # prefix)
...
std::vector<std::string> Tokenizer::bpe(const std::string& word) const {
    // Start with individual characters (already in Unicode representation)
...
std::vector<int> Tokenizer::encode(std::string_view text) const {
    ...
    // For each "word", convert bytes to Unicode tokens, apply BPE, look up IDs.
    size_t i = 0;
    while (i < text.size()) {
        // Collect a word: letters/digits, or a single space+letters, or punctuation
        std::string word;
...
std::string Tokenizer::decode(std::span<const int> tokens) const {
    ...
    // Concatenate token strings, then convert Unicode back to bytes
